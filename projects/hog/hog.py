...
def roll_dice(num_rolls, dice=six_sided):
    ...
    # END PROBLEM 1
    result = 0
    indicator = 0 # whether there is a 1
...
#print(play(lambda x,y:1,lambda x,y:1,dice=make_test_dice(2,5),goal=10))
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(score0,score1,last_score=last_score,running_high=running_high):
        ...
        if delta > running_high:
            print(f'{delta} point(s)! The most yet for Player {who}')
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    for num in range(1,11,1):
        a = make_averaged(roll_dice)(num,dice)
        if a>average_score:
            idx = num
            average_score=a
    return idx
