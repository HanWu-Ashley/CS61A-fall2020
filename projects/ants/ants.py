...
class Ant(Insect):
    ...
    def remove_from(self, place):
        if isinstance(place.ant,QueenAnt) and place.ant.is_true_queen:
            return 0 
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        distance = 0
        while (start_place is not beehive):
            #print('DEBUG:',start_place)
            if self.min_range<= distance<= self.max_range and start_place.bees != []:
                return rANTdom_else_none(start_place.bees)
            else:
    ...
    def reduce_armor(self, amount):
        ...
        if self.place.bees != []:
            for bee in self.place.bees[:]:
                Insect.reduce_armor(bee,amount + self.damage*int(self.armor <=0))
...
class HungryAnt(Ant):
    ...
    def eat_bee(self, bee):
        # BEGIN Problem 6
        "*** YOUR CODE HERE ***"
        assert self.digesting==0,'still digesting'
        if bee!=None:
...
# END Problem EC
    ...
    def action(self, gamestate):
        ...
        if not self.is_true_queen:
            ...
            # double the damage of all ants behind it
            start_place = self.place.exit
            while(start_place != None):
                ant = start_place.ant
                if (not ant in self.doubled_ants) and isinstance(ant,Ant):
                    ant.damage *= 2
