...
def subseqs(s):
    ...
    if s==[]:
        ...
        without_first_item = subseqs(s[1:])
        return without_first_item + insert_into_all(s[0],without_first_item) 
...
def inc_subseqs(s):
    ...
    last_lists = inc_subseqs(s[1:])
    can_be_inserted = [a for a in last_lists if (a==[] or a[0]>=s[0])]
    return last_lists + insert_into_all(s[0],can_be_inserted)
...
def shuffle(cards):
    ...
    for i in range(len(cards)):
        item = cards[int(i/2)] if i%2==0 else half[int((i-1)/2)]
...
def insert(link, value, index):
    ...
    if index==0 and link!=Link.empty:
        old_link = Link(link.first,rest = link.rest)
        link.first,link.rest = value,old_link
        return None
