...
def pingpong(n):
    ...
    def iter(index,total,dir):
        if index==n:
            return total
        if index%8==0 or num_eights(index)>0:
            return iter(index+1,total-dir,-dir)
        return iter(index+1,total+dir,dir)
    return  iter(1,1,1) 
...
def count_coins(total):
    ...
    def ways(smallest_coin,remain_coins):
        # number of ways to make change for remain_coins such that 最小的钱数>=smallest_coin
        ...
        '''
        用不少于smallest coin的钱分remain coins的方法数 
        = 当前方法至少存在一个零钱等于smallest coin的方法数 
           + 当前方法不存在任意一个零钱等于smallest coin（即用不少于next smallest的钱分remain coins）的方法数
        '''
